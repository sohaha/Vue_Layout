  ...
  methods: {
    ...
    addDragLogic(i, resizer) {
      ...
      const eDownFunc = e => {
        ...
        const onMouseMove = e => {
          ...
          const coor = this.split === 'horizontal' ? eMove.pageX : eMove.pageY;
    ...
    updateFracs() {
      ...
      for (let i = 0, j = 1; i < this.pos.length; i++, j++) {
        const iPos = this.pos[i];
        const jPos = j === this.pos.length ? 1 : this.pos[j];
        this.fracs.push(jPos - iPos);
      }
    },
    applyFracs() {
      const getStyleStr = frac => {
        const dim = this.split === 'horizontal' ? 'width' : 'height';
